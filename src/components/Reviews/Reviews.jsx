...
const Reviews = () => {
    ...
    return <ul className={css.blockReviews}>
        {detailsCar && detailsCar.reviews.map(({reviewer_name, reviewer_rating, comment}) => <li>
            <div className={css.blockInfo}>
                <div className={css.img}><p className={css.textName}>{reviewer_name[0]}</p></div>
                <div className={css.blockNameRating}>
                <p className={css.name}>{reviewer_name}</p>
                    <div className={css.blockRatingIcon}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
<g clip-path="url(#clip0_661_23336)">
...
<p>{reviewer_rating}</p>
            ...
            <p className={css.decr}>{comment}</p>
        </li>)}
    </ul>
}
